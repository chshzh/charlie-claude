...
> ProductManager → openspec → Developer → QA cycle.
...
## Post-Review: Feed Findings Back to openspec

After generating QA.md, convert every P1 and P2 issue into an openspec proposal.
This ensures each finding becomes a scoped, tracked change with pass criteria
before entering the development queue.

```bash
cd pm/
# Paste the issue title verbatim from QA.md:
openspec propose "fix SoftAP inactivity timeout drops headset during quiet audio"
openspec propose "add headset role to CI build matrix"
openspec propose "move CONFIG_BUF_WIFI_RX_PACKET_NUM to Kconfig"
```

Update `pm/QA.md` issue status (`P1-001 → In Progress / Resolved`) as proposals
are approved and implemented. See [`../WORKFLOW.md`](../WORKFLOW.md) for the
full phase diagram.
